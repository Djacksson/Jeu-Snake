...
def fin():
    can.delete(ALL)
    if manger>manger1:
        end=can.create_text(250,160,font=('Fixedsys',20),text="GAGNANT \nAvec un score de " +str(manger), fill="RED")
        end=can.create_text(950,160,font=('Fixedsys',20),text="PERDANT \nAvec un score de " +str(manger1), fill="RED")
    if manger<manger1:
        end1=can.create_text(250,160,font=('Fixedsys',20),text="PERDANT \nAvec un score de " +str(manger), fill="RED")
        end1=can.create_text(950,160,font=('Fixedsys',20),text="GAGNANT \nAvec un score de " +str(manger1), fill="RED")
    if manger==manger1:
        end2=can.create_text(650,160,font=('Fixedsys',20),text="EGALITE \nAvec un score de " +str(manger), fill="RED")
###############################################################################################################
def changecolor():
...
'#############################################################################'
#Apparution des pommes pour le joueur1
'#############################################################################'
def pose_pomme():
    global coord_pomme,x_pomme,y_pomme,pomme
    x_pomme=randrange(10,590,10)
...
'#############################################################################'
#Apparution des pommes pour le joueur2
'#############################################################################'
def pose_pomme1():
    global coord_pomme1,x_pomme1,y_pomme1,pomme1
    x_pomme1=randrange(610,1190,10)
...
def move():
    ...
    global x_pomme,y_pomme,corps,pos_x,pos_y,x,y,long,n,manger,delai, delai1, acceleration, Score, jouer, play            

    if temps == 2:
        jouer=can.create_text(630,200,font=('Fixedsys',25),text="Espace pour commencer !", fill="RED")
...
#######################################################################
    ...
    '#######################################################################'
    #Cadre autorisé au serpent du joueur 1
    '#######################################################################'
    if pos_y[long] > 440 or pos_y[long] < 10:
        fin()
    if pos_x[long] > 570 or pos_x[long] <  10:
        fin()
...
def move1():
    ...
    global x_pomme1,y_pomme1,corps1,pos_x1,pos_y1,x1,y1,long_1,n1,manger1, delai1, delai, acceleration2, Score1, jouer, play            
...
#######################################################################
    ...
    '#######################################################################'
    #Cadre autorisé au serpent du joueur 2
    '#######################################################################'
    if pos_y1[long_1] > 440 or pos_y1[long_1] < 10:
        fin()
    if pos_x1[long_1] > 1180 or pos_x1[long_1] < 610:
        fin()
...
def new_game():
    ...
    can.create_rectangle(10,0,1200,10,fill="#73aadc")
    can.create_rectangle(1190,0,1200,500,fill="#73aadc")
    can.create_rectangle(0,500,1200,460,fill="#73aadc")
    can.create_rectangle(590,0,600,460,fill="#73aadc")
...
#taille et couleur de font de la fenetre de jeu
can=Canvas(Fenetre, width=1200, height=500, bg="#a5a5a5",bd=0)    
can.pack()
...
Button_play.place(x=535, y=465, width=120, height=30)

Bouton_quit=Button(Fenetre,font=('Fixedsys',10,'bold'),bg='#fcbf7c',fg='#646464', text="Quitter", command=quitter)
Bouton_quit.place(x=1090, y=465, width=100, height=30)
...
Score.place(x=210, y=465, width=120, height=30)

Score1 = Label(can, font=('Fixedsys',10,'bold'),bg='#fcbf7c',fg='#646464',bd=2,relief='raised', text="Score : " +str(manger1),anchor=W)
Score1.place(x=880, y=465, width=120, height=30)
