...
def fin():
    can.delete(ALL)
    end=can.create_text(350,160,font=('Fixedsys',20),text="Vous êtes mort !!! \n\nVotre score est de : " +str(manger), fill="RED")   #fin du jeu
#######################################################################
# Les couleurs de la tete et du corp sont aléatoires ------------------
#######################################################################
def changecolor():
    global color_tete, color_corp   #class permettant de definir la couleur du corps et de la tete du snake
...
def tp():
    ...
    x_tp=randrange(60,440,10)
    y_tp=randrange(60,400,10)
    ...
    x_tp1=randrange(450,630,10)
    y_tp1=randrange(60,400,10)
...
# Boucles permettant le déplacement du serpent
    ...
    long=long-1 ## Dans une liste on part de 0 et que la longueur donne le nombre d'éléments;Il faut enlever 1
    ...
    if coord_tete==coord_pomme: ## Si les coordonnées de la tête et de la pomme sont les mêmes on fait la boucle
        ...
        ## Options du jeu
        if manger >= 100:
            can.configure(bg="#8080ff") ## Si manger >=100 le fond devient bleu clair
        if manger >= 200:
            can.configure(bg="#c0c0c0") ## Si manger >=100 le fond devient Gris clair
        if manger == 100:
            delai -= 5 ## Quand manger == 100 la vitesse du Serpent augmente
            ## Texte pour anoncer l'accélération du Serpent
            acceleration=can.create_text(350,450,font=('Fixedsys',16),text="Attention ! Petite accélération du serpent.", fill="RED") 
        if manger == 200:
            delai -= 5 ## Quand manger == 200 la vitesse du Serpent augmente
            ## Texte pour anoncer l'accélération du Serpent
            acceleration1=can.create_text(350,450,font=('Fixedsys',16),text="Vous êtes très fort, la difficulté augmente.", fill="RED")
        if manger > 100:
...
def new_game():
    ...
    #cadre de jeu au chaque nouvelle
...
def score_text():
    ...
    os.popen("scores_50.txt")
    temps1 = 1
    temps = 1
def score():
    global pseudo, temps, pause, temps1, manger, top
    score = open("scores_50.txt", "a")
...
def En_Savoir_Plus():#Fenetre pour afficher le "En_Savoir_Plus"
    global temps, temps1, pause
...
#######################################################################
Fenetre = Tk()
menubar = Menu(Fenetre)     #fenetre de l'espace de jeu
Fenetre.title("Best Snake") 
Fenetre.resizable(width=False,height=False)   #possibilité d'agranissement de la fenetre
#Dimension et position de l'interface de jeu
width_inter = 700
height_inter = 500
